...
import {
  ...
  Printer, MapPin, Phone, ClipboardList
} from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl border border-primary/20 bg-primary/5 flex-wrap">
          ...
          <div className="flex items-center gap-2 ml-auto flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl border border-violet-500/20 bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 transition-all font-bold"
            >
              <Printer className="w-3 h-3" /> Paket Listesi
            </button>
            <button
      ...
      {packingListOpen && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 bg-black/70 flex items-start justify-center p-4 overflow-y-auto print:p-0 print:bg-white">
            <div className="bg-slate-900 rounded-2xl border border-border/80 w-full max-w-3xl my-6 print:my-0 print:rounded-none print:border-0 print:bg-white print:text-black">
              {/* Header — hidden on print */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-border/60 print:hidden">
                <div className="flex items-center gap-2.5">
                  <ClipboardList className="w-5 h-5 text-violet-400" />
                  <h2 className="font-black text-foreground text-sm">
                    Paket Listesi — {selectedOrders.length} Sipariş
                  </h2>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => window.print()}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-500/10 text-violet-400 border border-violet-500/20 hover:bg-violet-500/20 transition-all text-xs font-bold"
                  >
              ...
              <div className="hidden print:block px-6 pt-4 pb-2 border-b border-gray-300">
                <div className="flex items-center justify-between">
                  <div>
                    <h1 className="text-xl font-black text-black">CanayanWeb — Paket Listesi</h1>
                    <p className="text-sm text-gray-600 mt-0.5">{printDate} · {selectedOrders.length} sipariş</p>
                  </div>
                  <div className="text-right text-sm text-gray-500">
                    <p className="font-bold">Hazırlayan: ____________</p>
                    <p className="mt-1">İmza: ____________</p>
                  </div>
                </div>
              </div>

              {/* Orders list */}
              <div className="p-6 space-y-3 print:p-4 print:space-y-2">
                {selectedOrders.map((o, idx) => {
                  const sc = STATUS_CONFIG[o.status] ?? { label: o.status, color: '' }
                  return (
                    <div
                      key={o.id}
                      className="border border-border/60 rounded-xl p-4 bg-slate-800/20 print:border-gray-300 print:rounded-none print:bg-white print:p-3 print:break-inside-avoid"
                    >
                      <div className="flex items-start justify-between gap-3">
                        {/* Order number + check box */}
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          <div className="w-6 h-6 border-2 border-slate-600 rounded print:border-black shrink-0 mt-0.5 print:w-5 print:h-5" />
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-black text-xs text-foreground font-mono print:text-black">
                                #{idx + 1} — {o.id.slice(0, 8).toUpperCase()}
                              </span>
                              <span className={`text-[9px] px-1.5 py-0.5 rounded-full border font-black uppercase print:hidden ${sc.color}`}>
                                {sc.label}
                              </span>
                            </div>
                            <p className="text-sm font-black text-foreground mt-0.5 print:text-black">{o.recipientName}</p>
                            {o.phone && (
                              <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5 print:text-gray-600">
                                <Phone className="w-3 h-3" />{o.phone}
                              </p>
                            )}
                            {(o.city || o.addressLine) && (
                              <p className="text-xs text-slate-400 flex items-start gap-1 mt-0.5 print:text-gray-600">
                                <MapPin className="w-3 h-3 shrink-0 mt-0.5" />
                                <span>{[o.addressLine, o.district, o.city].filter(Boolean).join(', ')}</span>
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="text-right shrink-0">
                          <p className="font-black text-base text-foreground font-mono print:text-black">
                            ₺{(o.totalAmount ?? 0).toLocaleString('tr-TR', { maximumFractionDigits: 0 })}
                          </p>
                          <p className="text-[10px] text-slate-500 font-semibold print:text-gray-500 mt-0.5">
                            {o.itemCount ?? '?'} ürün
                          </p>
                          {o.trackingNumber && (
                            <p className="text-[10px] text-cyan-400 font-mono mt-0.5 print:text-gray-700">
                      ...
                      <div className="mt-2.5 pt-2 border-t border-border/40 print:border-gray-300">
                        <p className="text-[10px] text-slate-600 print:text-gray-400 font-semibold">
                          Not / Kontrol: _______________________________________________
                        </p>
                      </div>
                    </div>
                  )
              ...
              {/* Summary footer */}
              <div className="px-6 py-4 border-t border-border/60 bg-slate-950/20 rounded-b-2xl print:border-t-gray-300 print:bg-white print:text-black">
                <div className="flex items-center justify-between text-xs font-bold">
                  <span className="text-slate-400 print:text-gray-600">
                    {selectedOrders.length} sipariş · Toplam {selectedOrders.reduce((s, o) => s + (o.itemCount ?? 0), 0)} ürün paketi
                  </span>
                  <span className="font-black text-foreground font-mono print:text-black">
                    ₺{selectedOrders.reduce((s, o) => s + (o.totalAmount ?? 0), 0).toLocaleString('tr-TR', { maximumFractionDigits: 0 })}
                  </span>
                </div>
              </div>
            </div>

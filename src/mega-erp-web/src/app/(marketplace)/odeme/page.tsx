...
export default function OdemePage() {
  ...
  const handleApplyCoupon = async () => {
    ...
    setCouponError('')
    try {
      const result = await marketplaceService.validateCoupon(couponCode.trim(), total)
      if (result.valid) {
        setCouponDiscount(result.discountAmount)
        setAppliedCoupon(couponCode.trim().toUpperCase())
        setCouponError('')
      } else {
        setCouponError(result.message ?? 'Kupon kodu geçersiz.')
        setCouponDiscount(0)
            ...
            {couponDiscount > 0 && (
              <div className="flex justify-between text-green-400">
                <span className="flex items-center gap-1">
                  <Tag className="w-3 h-3" /> Kupon ({appliedCoupon})
                </span>
            ...
            <div className="flex justify-between font-bold text-foreground pt-1 border-t border-border">
              ...
              <span className="text-primary">
                {(() => {
                  ...
                  return Math.max(0, cardTotal + extra - couponDiscount).toLocaleString('tr-TR', { style: 'currency', currency: 'TRY' })
                })()}
              </span>
            </div>
          </div>
